...
const downloadCV = () => {
  ...
  return (
    <section id="home" className="relative min-h-screen text-[#111111] dark:text-[#f0f0f0] selection:bg-blue-500/20 transition-colors duration-500 overflow-hidden">
      <nav className="fixed top-0 w-full z-50 backdrop-blur-md px-6 py-6 bg-white/90 dark:bg-black/90">
            ...
            <div className="flex flex-col items-start text-left mb-20 space-y-8">
              <div>
                <h1 className="text-5xl md:text-8xl font-bold tracking-tighter text-black dark:text-white leading-none">
                  <span ref={headingRef} className="inline-block opacity-0">Makadii I'm <span className="relative inline-block ml-1 md:ml-2">
                ...
                <div ref={statsRef} className="flex items-center gap-3 text-gray-500 font-medium text-sm md:text-lg opacity-0 mt-4">
              ...
              <div ref={socialRef} className="flex flex-wrap items-center gap-4 sm:gap-6 text-xl opacity-0">
                ...
                <div className="inline-flex items-center gap-3 px-3 py-1.5 bg-gray-400/5 dark:bg-white/[0.03] border border-black/5 dark:border-white/5 rounded-full whitespace-nowrap shrink-0">
          ...
          <div className="md:col-span-1 flex justify-center md:justify-end pt-8 md:pt-0">
            <div ref={profileImageRef} className="relative opacity-0">
              <div className="w-64 h-64 md:w-72 md:h-72 rounded-full overflow-hidden border-4 border-black dark:border-white/20 shadow-2xl">
                <img src={portfolioData.personal.profileImage || "https://via.placeholder.com/300/667eea/ffffff?text=Misheck"} alt="Misheck Gogo" className="w-full h-full object-cover" />
              </div>
